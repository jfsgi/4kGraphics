...
import { GRAIN_MM_U, GRAIN_MM_V } from './grain.js';
...
export function archedBoardGeometry(
  ...
  if (arch === 'waterfall') {
    ...
    // endSkew overrides the flat-low length at the front (so a jointed scoop's
    // middle section can align its ramp with the corner joint boards); else a
    // short default.
    const frontFlat = Math.min(endSkew > 0 ? endSkew : run * 0.28, run * 0.9);
 ...
 */
/** Orient a canonical French-dovetail key (depth +X, flare +Y, run +Z) so its
 *  depth lies along `interfaceAxis` and its run/slide along `runAxis`. The
 *  default runAxis 'z' reproduces the legacy vertical rail dovetail (and the
 *  old `interfaceAxis === 'y'` rotateZ). 'y' gives a shelf's front-to-back run. */
export function orientFrenchDovetail(
  geo: THREE.BufferGeometry,
  interfaceAxis: 'x' | 'y' | 'z',
  runAxis: 'x' | 'y' | 'z',
  runSign: 1 | -1 = 1,
): void {
  const unit = { x: [1, 0, 0], y: [0, 1, 0], z: [0, 0, 1] } as const;
  const depth = new THREE.Vector3(...unit[interfaceAxis]);
  const run = new THREE.Vector3(...unit[runAxis]).multiplyScalar(runSign);
  const flare = new THREE.Vector3().crossVectors(run, depth);
  geo.applyMatrix4(new THREE.Matrix4().makeBasis(depth, flare, run));
}
